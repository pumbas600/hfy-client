...
export default function About() {
  return (
    <PageLayout headerContent={<h2>About</h2>}>
      <TextLayout>
        <h3>{config.title}</h3>
        <p>
        ...
        <p>
          ...
          possible. It automatically parses HFY Reddit posts so that authors can
          continue posting on Reddit as normal. To achieve this, certain
          metadata is extracted from Reddit posts:
        </p>
        <ul>
          <li>
            <strong>First link</strong> — This is extracted from a link to a
            Reddit post with <em>first</em> in the label. The first link is used
            to quickly determines whether chapters belong to the same story.
          </li>
          <li>
        ...
        <p>
          ...
          automatically added where possible. When a broken link is found (such
          as a link to a chapter, that doesn’t link back to it) then a warning
          is raised, but currently, <strong>nothing is done</strong>. Any other
          links to Reddit posts are changed to instead link to this website.
        </p>
        <p>
          Periodically, chapters are synchronised with their corresponding
          Reddit post. The time between synchronisations is proportional to how
          old the post is. As a result, any upvotes or edits to the original
          Reddit post—including removing it all together—get reflected here.
        </p>
        <h3>Limitations</h3>
        <p>
          ...
          possible. Instead, the requests that can be made to the Reddit API are
          spent processing new posts, and regularly checking old posts for new
          updates.
        </p>
      </TextLayout>
    </PageLayout>
